...
class Conv2dBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, padding=0, pad_type='reflect', bias=True, norm_layer=None, nl_layer=None):
        ...
        self.conv = nn.Sequential(
            #add two 5*5fliter
            nn.Conv2d(in_planes, in_planes, kernel_size=5, stride=1, padding=2,bias=bias),
            nn.Conv2d(in_planes, in_planes, kernel_size=5, stride=1, padding=2, bias=bias),
            spectral_norm(nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
                                    padding=0, bias=bias))
        )'''''
...
class Conv2dBlock_last(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, padding=0, pad_type='reflect', bias=True,
                 norm_layer=None, nl_layer=None):
        super(Conv2dBlock_last, self).__init__()
        if pad_type == 'reflect':
            self.pad = nn.ReflectionPad2d(padding)
        elif pad_type == 'replicate':
            self.pad = nn.ReplicationPad2d(padding)
        ...
        self.conv = nn.Sequential(
            #add one 5*5fliter
            #spectral_norm(nn.Conv2d(in_planes, in_planes, kernel_size=5, stride=1, padding=2,bias=bias)),
            spectral_norm(nn.Conv2d(in_planes, in_planes, kernel_size=5, stride=1, padding=2, bias=bias)),
            spectral_norm(nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
                                    padding=0, bias=bias))
        )
...
class SingleGenerator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, nc=2, e_blocks=6, norm_type='instance', up_type='Trp'):
        ...
        block += [Conv2dBlock (ngf, ngf, kernel_size=5, stride=1, padding=2, pad_type='reflect', bias=False,nl_layer=nn.Tanh)]
        block += [Conv2dBlock(ngf, output_nc, kernel_size=5, stride=1, padding=2, pad_type='reflect', bias=False, nl_layer=nn.Tanh)]
